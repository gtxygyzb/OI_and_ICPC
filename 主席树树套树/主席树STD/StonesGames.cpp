...
int main()
{
    ...
    for (int lp, rp, l, r; q--; )
    {
        ...
        while (1)
        {
            num = query1(rt[r], rt[l - 1], sum + 1, 1e9, 1, 1e9); // 返回第一个大于等于sum + 1的数,找不到就返回-1
            sum = query2(rt[r], rt[l - 1], 1, num  == -1 ? 1e9 : num - 1, 1, 1e9); // 除这个数以外从小到大的前缀和
            if (num == -1 || sum + 1 < num) //找不到了，此时sum为区间和；或者加起来不够
            {
                ans = sum + 1;
                break;
            }
            else sum += num; //加上这个数，接着循环，最多log次
        }
        printf("%lld\n", ans);
    }
    return 0;
