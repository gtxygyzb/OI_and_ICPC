...
void change(int u, int st, int ed, int l, int r, int val)
{
    ...
    if (st <= mid)
    {
        if (st == l && ed == r) change(lc, l, mid, l, mid, val);
        else change(lc, st, min(mid, ed), l, mid, val);
    }
    if (ed > mid)
    {
        if (st == l && ed == r) change(rc, mid + 1, r, mid + 1, r, val);
        else change(rc, max(st, mid + 1), ed, mid + 1, r, val);
    }
    pu(u);
}
